...
test.describe('UI – Structure & Layout', () => {
  ...
  test('TC-UI-01: page title is "Scientific Calculator"', async ({ calc }) => {
    await expect(calc.page).toHaveTitle('Scientific Calculator');
  });

  test('TC-UI-02: display is visible and starts empty', async ({ calc }) => {
    await expect(calc.display).toBeVisible();
    expect(await calc.getDisplay()).toBe('');
  });

  test('TC-UI-03: display is read-only (disabled input)', async ({ calc }) => {
    await expect(calc.display).toBeDisabled();
  });

  test('TC-UI-04: all 24 buttons are present', async ({ calc }) => {
    await expect(calc.page.getByRole('button')).toHaveCount(24);
  });

  for (const label of expectedButtons) {
    test(`TC-UI-05: button "${label}" is visible`, async ({ calc }) => {
      await expect(
  ...
  test('TC-UI-06: clear button resets display to empty', async ({ calc }) => {
  ...
  test('TC-UI-07 [BUG-001] button "3" must append digit 3, not 0', async ({ calc }) => {
  ...
  test('TC-UI-08 [BUG-002] button "−" must append "-", not "/"', async ({ calc }) => {
